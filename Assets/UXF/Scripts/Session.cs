...
namespace UXF
{
    ...
    public class Session : MonoBehaviour, IExperimentUnit, IDataAssociatable
    {
        ...
        public void Begin(string experimentName, string participantId, int sessionNumber = 1, Dictionary<string, object> participantDetails = null, Settings settings = null)
        {
            ...
            // Precompile critical code to reduce JIT for better performance
            Prewarm();
        ...
        public void Prewarm()
        {
            Utilities.UXFDebugLog("Pre-compiling critical code.");

            // Triggers compilation of Trial.End() without running it.
            // This doesn't seem to work.
            // var functionPointer = typeof(Trial).GetMethod("End").MethodHandle.GetFunctionPointer();
            // typeof(Trial).GetMethod("SaveData").MethodHandle.GetFunctionPointer(); // is private
            // functionPointer = typeof(Trial).GetMethod("SaveDataTable").MethodHandle.GetFunctionPointer();

            // Actually call the functions to force pre-compilation.

            // public void SaveDataTable(UXFDataTable table, string dataName, UXFDataType dataType = UXFDataType.OtherTrialData)
            var dummyBlock = CreateBlock();
            var dummyTrial = dummyBlock.CreateTrial();
            dummyTrial.result = new ResultsDictionary(Headers, true);

            var dataTable = new UXFDataTable(100, "time", "x", "y", "z");

            // Session.SaveDataTable
            this.SaveDataTable(dataTable, "tempTable", UXFDataType.TrialResults);
            this.SaveDataTable(dataTable, "tempTable", UXFDataType.ParticipantDetails);
            this.SaveDataTable(dataTable, "tempTable", UXFDataType.SessionLog);

            // Trial.SaveDataTable
            dummyTrial.SaveDataTable(dataTable, "tempTable", UXFDataType.Trackers);
            ...
            // foreach (var dataHandler in ActiveDataHandlers)
            // {
            //     Utilities.UXFDebugLog("Pre-compiling HandleDataTable");
            //     // dataHandler.HandleDataTable(dataTable, "dummyPrecompile", "dummy", -1, "dummyPrecompile.txt", UXFDataType.OtherTrialData);
            // }
